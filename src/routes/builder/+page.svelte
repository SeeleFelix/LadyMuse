<script lang="ts">
	...
	function getTechniquesForSlot(slotKey: string): Technique[] {
		...
		const selectedIds = new Set(Object.values(slots).flat().map((i) => i.technique.id));
	...
	function addToSlot(slot: string, tech: Technique) {
		...
		slots[slot] = [...slots[slot], { technique: tech, weight: tech.weightHint ?? 1.0 }];
	}
	...
	let negativePrompt = $derived.by(() => {
		...
		for (const items of Object.values(slots)) {
			for (const item of items) {
				if (item.technique.negativeKeywords) parts.push(item.technique.negativeKeywords);
			}
		}
	...
	function buildTagPrompt(): string {
		...
		for (const items of Object.values(slots)) {
			for (const item of items) {
				...
				if (item.weight !== 1.0) {
					const weighted = kws.split(',').map((k) => `(${k.trim()}:${item.weight})`).join(', ');
					parts.push(weighted);
				} else {
					parts.push(kws);
				}
	...
	function buildNLPrompt(): string {
		...
		if (selectedStyle?.nlTemplate) {
			...
			nlParts.push(`An image of ${subjectText} in ${selectedStyle.nameZh || selectedStyle.name} style`);
		} else {
...
<div class="flex h-full">
	...
	<div class="w-80 shrink-0 border-r border-zinc-800 bg-zinc-900/50 overflow-y-auto p-4">
		...
		{:else}
			{#each Object.entries(slotLabels) as [slotKey, label]}
				...
				{@const filtered = searchQuery ? techs.filter((t) => (t.nameZh || t.name).includes(searchQuery) || t.promptKeywords.toLowerCase().includes(searchQuery.toLowerCase())) : techs}
					...
					<div class="mt-4">
						...
						<div class="flex flex-wrap gap-1">
							...
							{#if filtered.length > 20}
								<span class="text-xs text-zinc-600">+{filtered.length - 20} 更多</span>
							{/if}
						</div>
					</div>
				{/if}
	...
	<div class="flex-1 overflow-y-auto p-6">
		<div class="flex items-center justify-between">
			...
			<div class="flex gap-1 rounded-lg border border-zinc-700 p-1">
				<button
					...
					class="rounded-md px-3 py-1 text-xs {promptMode === 'tag' ? 'bg-violet-600 text-white' : 'text-zinc-400 hover:text-zinc-200'}"
				>标签 (SD)</button>
				<button
					onclick={() => promptMode = 'nl'}
					class="rounded-md px-3 py-1 text-xs {promptMode === 'nl' ? 'bg-violet-600 text-white' : 'text-zinc-400 hover:text-zinc-200'}"
				>自然语言 (FLUX)</button>
			</div>
		</div>
		...
		<div class="mt-6">
			...
			<textarea
				bind:value={subject}
				placeholder="{promptMode === 'nl' ? '用自然语言描述场景... 例如：一个女人站在古老的教堂中殿，阳光透过彩色玻璃窗洒落' : '描述你的主体... 例如：一个站在古教堂里的女人'}"
				rows="2"
		...
		<div class="mt-4">
			...
			<div class="mt-1 flex flex-wrap gap-2">
				<button
					...
					class="rounded-lg border px-3 py-2 text-xs {!selectedStyle ? 'border-violet-500 bg-violet-600/20 text-violet-300' : 'border-zinc-700 bg-zinc-800 text-zinc-400 hover:border-zinc-600'}"
				>无</button>
				{#each allStyles as style}
					<button
						onclick={() => selectedStyle = selectedStyle?.id === style.id ? null : style}
						class="rounded-lg border px-3 py-2 text-xs {selectedStyle?.id === style.id ? 'border-violet-500 bg-violet-600/20 text-violet-300' : 'border-zinc-700 bg-zinc-800 text-zinc-400 hover:border-zinc-600'}"
				...
				<div class="rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
					...
					<div class="mt-2 flex flex-wrap gap-1.5">
						{#each slots[slot] as item (item.technique.id)}
							<span class="inline-flex items-center gap-1 rounded bg-violet-600/20 px-2 py-1 text-xs text-violet-300">
		...
		<div class="mt-6 rounded-lg border border-violet-600/30 bg-zinc-900 p-4">
			...
			<pre class="whitespace-pre-wrap text-xs text-zinc-300 leading-relaxed">{positivePrompt || '开始添加元素来构建提示词...'}</pre>
		</div>
		...
		{#if negativePrompt}
			<div class="mt-3 rounded-lg border border-zinc-700 bg-zinc-900 p-4">
				...
				<pre class="whitespace-pre-wrap text-xs text-red-300/70 leading-relaxed">{negativePrompt}</pre>
			</div>
		{/if}
	</div>
</div>
