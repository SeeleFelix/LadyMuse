<script lang="ts">
	...
	async function generateMood(mood: string) {
		loading = true;
		lastMood = mood;
		const res = await fetch(`/api/inspiration?mode=mood&mood=${mood}&prompt_mode=${promptMode}`);
		if (res.ok) result = await res.json();
		loading = false;
	}
	...
	async function generateMix() {
		loading = true;
		lastMood = '';
		const res = await fetch(`/api/inspiration?mode=mix&prompt_mode=${promptMode}`);
		if (res.ok) result = await res.json();
		loading = false;
	}
	...
	function copy(text: string, type: 'pos' | 'neg') {
		...
		if (type === 'pos') { copiedPos = true; setTimeout(() => copiedPos = false, 2000); }
		else { copiedNeg = true; setTimeout(() => copiedNeg = false, 2000); }
	}
</script>
...
<div class="p-8 max-w-4xl mx-auto">
	...
	<div class="mt-6 flex items-center gap-3">
		...
		<button
			...
			class="rounded-lg px-3 py-1.5 text-sm {promptMode === 'tag' ? 'bg-violet-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}"
		>标签模式 (SD1.5/SDXL)</button>
		<button
			onclick={() => promptMode = 'nl'}
			class="rounded-lg px-3 py-1.5 text-sm {promptMode === 'nl' ? 'bg-violet-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}"
			...
			<div class="flex flex-wrap gap-2">
				{#each Object.entries(moods) as [key, label]}
					<button
						...
						disabled={loading}
						class="rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2 text-sm text-zinc-300 transition-colors hover:border-violet-500 hover:text-violet-300 disabled:opacity-50 {lastMood === key ? 'border-violet-500 text-violet-300 bg-violet-600/10' : ''}"
					>{label}</button>
				{/each}
			</div>
		</div>
	...
	{#if result}
		<div class="mt-8 space-y-4">
			<!-- Meta info -->
			<div class="flex flex-wrap gap-2">
				<span class="rounded-full bg-zinc-800 px-3 py-1 text-xs text-zinc-400">主体: {result.subject}</span>
				{#if result.style}
					<span class="rounded-full bg-zinc-800 px-3 py-1 text-xs text-zinc-400">
						风格: {result.style.name}{#if result.style.mixWith} + {result.style.mixWith}{/if}
					</span>
				{/if}
				{#each result.techniques as tech}
					<span class="rounded-full bg-violet-600/20 px-3 py-1 text-xs text-violet-300">{tech.name}</span>
				{/each}
			</div>
			...
			<div class="rounded-lg border border-violet-600/30 bg-zinc-900 p-4">
				...
				<pre class="whitespace-pre-wrap text-sm text-zinc-200 leading-relaxed">{result.prompt.positive}</pre>
			</div>
			...
			{#if result.prompt.negative}
				<div class="rounded-lg border border-zinc-700 bg-zinc-900 p-4">
					...
					<pre class="whitespace-pre-wrap text-sm text-red-300/70 leading-relaxed">{result.prompt.negative}</pre>
				</div>
			{/if}
		</div>
	{/if}
