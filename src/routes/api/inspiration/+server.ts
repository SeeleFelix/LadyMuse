...
function buildTagPrompt(
	...
	if (style) {
		const tpl = style.positiveTemplate.replace('{subject}', '').replace(/,\s*,/, ',').replace(/^,\s*/, '');
	...
	const negUnique = [...new Set(negParts.join(', ').split(',').map((s) => s.trim()).filter(Boolean))];
...
function buildNLPrompt(
	...
	if (style?.nlTemplate) {
		...
		nlParts.push(`An image of ${subjectText} in ${style.nameZh || style.name} style`);
	} else {
...
export const GET: RequestHandler = async ({ url }) => {
	...
	if (mode === 'mood' && mood) {
		...
		if (promptMode === 'tag' && otherStyle.positiveTemplate) {
			result.positive += ', ' + otherStyle.positiveTemplate.replace('{subject}', '').replace(/^,\s*/, '');
		...
		return json({
			subject,
			style: { name: selectedStyle.nameZh || selectedStyle.name, mixWith: otherStyle.nameZh || otherStyle.name },
			techniques: selectedTechs.map((t) => ({ name: t.nameZh || t.name, slug: t.slug })),
			prompt: result,
			mixInfo
		});
	} else {
	...
	return json({
		subject,
		style: selectedStyle ? { name: selectedStyle.nameZh || selectedStyle.name } : null,
		techniques: selectedTechs.map((t) => ({ name: t.nameZh || t.name, slug: t.slug })),
		prompt: result
	});
};
