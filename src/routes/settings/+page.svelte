<script lang="ts">
	...
	let models = $state<{ id: string; name: string; provider: string; description?: string }[]>([]);
	...
	async function loadConfig() {
		...
		if (res.ok) {
			...
			favoriteModels = config.favorite_models ? JSON.parse(config.favorite_models) : [];
		}
	}
	...
	async function saveKey(providerId: string, value: string) {
		...
		message = '';
		const configKey = providerId === 'openrouter' ? 'openrouter_api_key' : 'deepseek_api_key';
		try {
			const res = await fetch('/api/config', {
				method: 'POST',
			...
			message = res.ok ? 'API Key 已保存' : '保存失败';
		} catch {
			message = '网络错误';
		}
		savingProvider = '';
	...
	async function toggleFavorite(modelId: string) {
		...
		await fetch('/api/config', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ key: 'favorite_models', value: JSON.stringify(favoriteModels) })
		});
	}

	const providerSections = [
		{ id: 'openrouter', name: 'OpenRouter', key: () => openrouterKey, setKey: (v: string) => openrouterKey = v, placeholder: 'sk-or-...' },
		{ id: 'deepseek', name: 'DeepSeek', key: () => deepseekKey, setKey: (v: string) => deepseekKey = v, placeholder: 'sk-...' }
	];
...
<div class="mx-auto max-w-3xl space-y-8 p-6">
	...
	{#if message}
		<p class="rounded-lg bg-zinc-800 px-4 py-2 text-sm text-violet-400">{message}</p>
	{/if}
	...
	{#each providerSections as p}
		<section class="space-y-3 rounded-xl border border-zinc-800 bg-zinc-900 p-5">
			...
			{#if models.filter((m) => m.provider === p.id).length > 0}
				<div class="max-h-48 space-y-1 overflow-y-auto border-t border-zinc-800 pt-3">
					{#each models.filter((m) => m.provider === p.id) as m}
						<div class="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-zinc-800">
							<button
								onclick={() => toggleFavorite(m.id)}
								class="text-sm {favoriteModels.includes(m.id) ? 'text-yellow-400' : 'text-zinc-600'} hover:text-yellow-300"
							...
							{/if}
							<button
								onclick={() => { defaultModel = m.id; saveDefaultModel(); }}
		...
		<section class="space-y-3 rounded-xl border border-zinc-800 bg-zinc-900 p-5">
			...
			<div class="flex flex-wrap gap-2">
				{#each favoriteModels as fid}
					...
					<span class="rounded-lg bg-zinc-800 px-3 py-1.5 text-sm text-zinc-300">
						{m?.name || fid}
					</span>
				{/each}
			</div>
