...
<div class="flex h-full">
	<div class="w-72 shrink-0 border-r border-zinc-800 bg-zinc-900/50 overflow-y-auto p-4">
		...
		{:else}
			{#each families as family}
				{#if family.styles.length > 0}
					<div class="mb-4">
						<h3 class="text-sm font-semibold text-zinc-300 mb-2">{family.nameZh || family.name}</h3>
						<div class="space-y-1">
							{#each family.styles as style}
								<button
									onclick={() => selectedStyle = style}
									class="block w-full text-left rounded px-2.5 py-1.5 text-sm {selectedStyle?.id === style.id ? 'bg-violet-600/20 text-violet-300' : 'text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200'}"
	...
	<div class="flex-1 overflow-y-auto p-6">
		{#if selectedStyle}
			<div>
				<h1 class="text-2xl font-bold text-zinc-100">{selectedStyle.nameZh || selectedStyle.name}</h1>
				<p class="mt-1 text-base text-zinc-500">{selectedStyle.name}</p>

				{#if selectedStyle.description}
					<p class="mt-3 text-sm text-zinc-400 leading-relaxed">{selectedStyle.description}</p>
				{/if}
				...
				<div class="mt-6 rounded-lg border border-zinc-800 bg-zinc-900 p-4">
					...
					<pre class="whitespace-pre-wrap text-xs text-violet-300">{selectedStyle.positiveTemplate}</pre>
				</div>
				...
				{#if selectedStyle.negativePrompt}
					<div class="mt-4 rounded-lg border border-zinc-800 bg-zinc-900 p-4">
						...
						<pre class="whitespace-pre-wrap text-xs text-red-300/70">{selectedStyle.negativePrompt}</pre>
					</div>
				{/if}
				...
				{#if selectedStyle.qualityTags}
					<div class="mt-4 rounded-lg border border-zinc-800 bg-zinc-900 p-4">
						...
						<div class="flex flex-wrap gap-1.5">
							{#each selectedStyle.qualityTags.split(',').map((k) => k.trim()).filter(Boolean) as tag}
								<span class="rounded bg-emerald-600/20 px-2.5 py-1 text-xs text-emerald-300">{tag}</span>
							{/each}
						</div>
					</div>
				{/if}
				...
				{#if selectedStyle.tags}
					<div class="mt-4">
						...
						<div class="flex flex-wrap gap-1.5">
							{#each selectedStyle.tags.split(',').map((t) => t.trim()).filter(Boolean) as tag}
