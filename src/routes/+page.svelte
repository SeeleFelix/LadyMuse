...
<div class="p-8">
	...
	<div class="mt-8 max-w-3xl mx-auto">
		<div class="rounded-xl border border-violet-500/30 bg-violet-600/5 p-6 text-center">
			<div class="flex justify-center gap-2 mb-4">
				<button
					onclick={() => quickMode = 'tag'}
					class="rounded-lg px-3 py-1 text-xs {quickMode === 'tag' ? 'bg-violet-600 text-white' : 'bg-zinc-800 text-zinc-400'}"
				>标签 (SD)</button>
				<button
					onclick={() => quickMode = 'nl'}
					class="rounded-lg px-3 py-1 text-xs {quickMode === 'nl' ? 'bg-violet-600 text-white' : 'bg-zinc-800 text-zinc-400'}"
				>自然语言 (FLUX)</button>
			</div>
			<button
				...
				<div class="mt-4 rounded-lg bg-zinc-900 p-4 text-left">
					<div class="flex items-center justify-between mb-2">
						<span class="text-xs text-zinc-500">
							{quickResult.style?.name || '随机风格'} · {quickResult.techniques?.map((t: any) => t.name).join(' · ')}
						</span>
						<button
							onclick={copyPrompt}
					...
					<pre class="whitespace-pre-wrap text-sm text-zinc-200 leading-relaxed">{quickResult.prompt.positive}</pre>
				</div>
			{/if}
		</div>
	</div>
	...
	<div class="mt-6 max-w-3xl mx-auto grid grid-cols-3 gap-3">
		<a href="/builder" class="group rounded-lg border border-zinc-800 bg-zinc-900 p-5 transition-colors hover:border-violet-600 hover:bg-zinc-800">
			<div class="text-xl">⚡</div>
			<div class="mt-2 font-semibold text-zinc-200 group-hover:text-violet-400">提示词构建器</div>
			<div class="mt-1 text-xs text-zinc-500">从艺术概念组装提示词</div>
		</a>
		<a href="/knowledge" class="group rounded-lg border border-zinc-800 bg-zinc-900 p-5 transition-colors hover:border-violet-600 hover:bg-zinc-800">
			<div class="text-xl">◈</div>
			<div class="mt-2 font-semibold text-zinc-200 group-hover:text-violet-400">知识探索器</div>
			<div class="mt-1 text-xs text-zinc-500">学习艺术词汇与技法</div>
		</a>
		<a href="/styles" class="group rounded-lg border border-zinc-800 bg-zinc-900 p-5 transition-colors hover:border-violet-600 hover:bg-zinc-800">
			<div class="text-xl">◆</div>
			<div class="mt-2 font-semibold text-zinc-200 group-hover:text-violet-400">风格画廊</div>
