...
好的锚点：masterpiece, best quality, anime screenshot, anime coloring, safe, @artist_name. An anime girl with medium-length blonde hair sits alone at a diner counter, shoulders hunched, hands wrapped around a cold cup.
...
**用标签写（元数据）**：
- quality 标签：根据画面风格从场景表中选择（见下方"质量标签场景表"），不要固定用同一套
- 安全标签：根据内容从场景表中选择（见下方"安全标签场景表"），必须包含一个
...
- **始终包含：** `worst quality, low quality, score_1, score_2, score_3, artist name`
- **有人物时叠加：** `bad hands, bad anatomy, missing fingers, extra digits, fewer digits, disfigured, mutation, 4 fingers, 6 fingers`
- **特写/肖像时叠加：** `bad face, ugly face, deformed face, cross-eyed`
- **水印/文字排除（按需）：** `watermark, signature, text, logo, jpeg artifacts, patreon logo, patreon username, twitter username, twitter logo`
- **风格排除（按需）：** `monochrome, sketch, multiple views, chibi, blurry, 3d`
- **构图控制（按需）：** `cropped, out of frame, multiple views, close-up`
- **风景/无人物：** 不叠加人物修正，改用 `person, human` 排除人物 + 水印排除

**默认回退**（无法判断时）：质量 `masterpiece, best quality` + 安全 `safe` + 负面 `worst quality, low quality, score_1, score_2, score_3, artist name`
...
prompt：
masterpiece, best quality, anime screenshot, anime coloring, safe, @big chungus. An anime girl with medium-length blonde hair sits alone at the far end of a long empty diner counter, shoulders hunched inward, hands wrapped around a cold coffee cup. Cold white fluorescent light from above bathes everything in flat, shadowless pale blue, the empty counter stretching away into the background. Skin texture visible, laminate countertop reflecting faint blue.

注意：
- 根据日式平涂风格选了 anime screenshot + anime coloring 而非 score 标签
